...
class TaskDetailScreen extends StatelessWidget {
  ...
  String _getDaysRemaining(String dateStr) {
    try {
      ...
      final target = DateTime(int.parse(parts[2]), int.parse(parts[1]), int.parse(parts[0]));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Detalles'), backgroundColor: Colors.indigo, foregroundColor: Colors.white),
      body: Column(
        children: [
          Expanded(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(task.title, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Colors.indigo)),
                  ...
                  Row(
                    children: [
                      ...
                      const Spacer(),
                      Chip(
                        label: Text(_getDaysRemaining(task.date), style: const TextStyle(color: Colors.white)),
